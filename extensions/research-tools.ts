...
import { registerBohrModelCommand } from "./research-tools/bohr-model.js";
import { installBohrHeader } from "./research-tools/header.js";
...
export default function researchTools(pi: ExtensionAPI): void {
	...
	pi.on("session_start", async (_event, ctx) => {
		await installBohrHeader(pi, ctx, cache);
	});

	registerAlphaTools(pi);
	registerDiscoveryCommands(pi);
	registerBohrModelCommand(pi);
